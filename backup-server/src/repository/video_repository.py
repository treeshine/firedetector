...
class VideoRepository:
    ...
    def get_backup_video_list(self):
        videos = self.db.query(Video).filter(Video.type == "BACKUP").order_by(Video.created_at.desc()).all()
        return videos
    def get_fp_video_list(self):
        videos = self.db.query(Video).filter(Video.type == "FP").order_by(Video.created_at.desc()).all()
        return videos
